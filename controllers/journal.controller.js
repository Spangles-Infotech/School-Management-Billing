...
export const journalEntry = async (req,res) => {
    ...
    try {
        ...
        if(req.method === "PUT"){
            const exisitingJournal = await Journal.findById(journalId)
            const data = await fetchHeadAccountAmount(headAccount)
            if(transactionMode !== exisitingJournal.transactionMode || amount !== exisitingJournal.amount){
                await ReceiptPaymentCalculation(exisitingJournal.rp, exisitingJournal.transactionMode, data.amount, exisitingJournal.amount, true)
            }
